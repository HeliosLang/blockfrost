          ...
          Effect.gen(function* () {
            ...
            while (true) {
              ...
              const response = yield* executeWithRateLimitRetry(
                () =>
                  client.get(
                    `${baseUrl}/addresses/${args.address}/transactions?${query}`,
                    {
                      headers: {
                        project_id: config.projectId
                      },
    ...
    Layer.effect(
      ...
      Effect.succeed(
        (
          ...
          Ledger.Tx.Tx,
          Network.ConnectionError | Network.UnexpectedFormat | Network.TxNotFound,
          never
        > =>
          executeWithRateLimitRetry(() =>
            client.get(`${baseUrl}/txs/${hash}/cbor`, {
              headers: {
                project_id: config.projectId
              },
              accept: "application/json"
            ...
            Effect.flatMap(response =>
              Effect.gen(function* () {
                ...
                const tx = yield* Ledger.Tx.decode({ trusted: true })(cbor).pipe(
                  Effect.provideService(
                    Network.UTxO,
                    getUTxO
                  ),
                ...
                if (Ledger.Tx.hash(tx) !== hash) {
                  return yield* Effect.fail(
                    new Network.UnexpectedFormat(
                      `Blockfrost tx hash mismatch (${Ledger.Tx.hash(tx)})`
                    )
                  )
                }
...
export const BlockfrostLayer = (config: Config) =>
